...
def _notify_progress(on_progress, done: int, total: int, url: str) -> None:
    if on_progress is None:
        return
    try:
        on_progress(int(done), int(total), str(url or ""))
    except Exception:
        pass


def _progress_total(done: int, queued: int) -> int:
    planned = done + max(queued, 0)
    cap = _MAX_EXTRA_TARGETS + 2
    return max(done, min(planned, cap))
...
def _budget_left(started: float) -> bool:
    return (time.time() - started) < _SCAN_BUDGET_SEC
...
def analyse_dynamic(
    ...
    *,
    ...
    try:
        from security_checks.injection_checks import set_scan_progress
        set_scan_progress(on_progress)
    except Exception:
        set_scan_progress = None
    try:
        return _analyse_dynamic_body(
            url,
            fetch_fn=fetch_fn,
...
def _analyse_dynamic_body(
    ...
    *,
    ...
    _notify_progress(on_progress, 1, 0, target)
    artefact = _fetch(
        target,
        browser=False,
    )
    ...
    while (
        ...
        _notify_progress(
            on_progress,
            len(visited),
            _progress_total(len(visited), len(queue)),
            extra_url,
        )
        extra_art = _fetch(
            extra_url,
