...
class TechStackPage(QWidget):
    ...
    def _make_empty_card(self, title: str, body: str) -> tuple[QFrame, QLabel]:
        """Simple empty-state card — single multi-line label to avoid wrap glitches."""
        card = QFrame()
    ...
    def init_ui(self):
        ...
        # ---- Platform evaluation (Get Stack findings) ----
        ...
        self.platform_hint = QLabel(
            "Hardening notes and platform findings from Get Stack. "
            "Scan vulnerabilities stay on the Alerts page."
        )
        ...
        self.platform_table.setColumnCount(7)
        self.platform_table.setHorizontalHeaderLabels(
            ["Risk", "Finding", "Standards", "Conf.", "Location", "Detail", "Remediation"]
        )
        ...
        self.platform_table.setColumnWidth(2, 160)
        ph.setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed)
        self.platform_table.setColumnWidth(3, 72)
        ph.setSectionResizeMode(4, QHeaderView.ResizeMode.Interactive)
        self.platform_table.setColumnWidth(4, 150)
        ...
        self.platform_table.verticalHeader().setDefaultSectionSize(96)
    ...
    def _fill_platform(self, findings: list):
        ...
        for i, f in enumerate(findings[:40]):
            ...
            self.platform_table.setItem(i, 2, std_item)
            ...
            self.platform_table.setItem(i, 3, conf_item)
            ...
            self.platform_table.setItem(i, 4, loc_item)
            ...
            self.platform_table.setItem(i, 5, det_item)
            ...
            self.platform_table.setItem(i, 6, rem_item)

            chars = max(len(detail), len(rem), 1)
            lines = max(std_lines, (chars // 55) + 1, 2)
