...
class DashboardPage(QWidget):
    ...
    def init_ui(self):
        ...
        nist_card = self._panel("NIST mapping (top)")
        nl = nist_card.layout()
        ...
        sans_card = self._panel("SANS mapping (top)")
        sl = sans_card.layout()
    ...
    def refresh(self):
        ...
        try:
            ...
            # Global dashboard (all users) — matches project scope charts
        ...
        # Findings by location (+ standards tooltip on CWE cell)
        ...
        # CWE / NIST / SANS tables (prefer DB aggregates; fallback to recent findings)
        ...
        if findings and (not cwe_counts or not nist_counts or not sans_counts):
            for f in findings:
        ...
        self._fill_count_table(
            self.cwe_table, cwe_counts, "No CWE data yet — run a scan", "#1d4ed8"
        )
        self._fill_count_table(
            self.nist_table, nist_counts, "No NIST data yet — run a scan", "#b45309"
        )
        self._fill_count_table(
            self.sans_table, sans_counts, "No SANS data yet — run a scan", "#9f1239"
        )
