...
# Local helpers below only support build_report_dict().
# Member 6 owns executive_summary, cwe/owasp summaries, findings, and remediation
# inside generate_report() return dict. GUI only renders report["remediation"].
...
def _default_remediation(findings: list) -> list:
    tips = []
    seen = set()
    for f in findings or []:
        name = str(f.get("vulnerability", "")).lower()
        cwe = str(f.get("cwe_id") or "")
        if "header" in name and "header" not in seen:
            tips.append("Add missing security headers (e.g. X-Frame-Options, CSP, HSTS).")
            seen.add("header")
        if "cookie" in name and "cookie" not in seen:
            tips.append("Set Secure and HttpOnly flags on all session cookies.")
            seen.add("cookie")
        if ("xss" in name or cwe == "CWE-79") and "xss" not in seen:
            tips.append("Encode output and enforce a strict Content-Security-Policy.")
            seen.add("xss")
        if ("sql" in name or cwe == "CWE-89") and "sql" not in seen:
            tips.append("Use parameterised queries; never concatenate user input into SQL.")
            seen.add("sql")
    if not tips:
        if not findings:
            tips.append(
                "No vulnerabilities were detected. Continue periodic scans and keep dependencies updated."
            )
        else:
            tips.append(
                "Review each finding and apply vendor / OWASP / CWE remediation guidance."
            )
    return tips


def build_report_dict(url: str, findings: list, scan_type: str = "Dynamic") -> dict:
    """
    Local structured builder used until Member 6 module is ready.
    Member 6 should return the SAME shape from generate_report().
    Uses core.cwe_map only while Member 1 is not yet providing full metadata.
    """
    try:
        from core.cwe_map import enrich_findings, cwe_summary, owasp_summary
    ...
    if summary["Total"] == 0:
        ...
        executive = (
            ...
            f"Findings are mapped to CWE/WASC/OWASP where applicable."
        )
    ...
    return {
        ...
        "remediation": _default_remediation(findings),
...
class ReportTab(QWidget):
    ...
    def init_ui(self):
        ...
        b_l.addWidget(title)
        self.meta_label = QLabel("Generate a report after a successful scan.")
        ...
        self.map_card = self._section_card("CWE / OWASP mapping summary")
        ...
        self.map_empty = QLabel(
            "No CWE/OWASP mappings — no issues found or not yet generated."
        )
        ...
        self.findings_card = self._section_card("Findings")
        ...
        self.findings_empty = QLabel(
            "No issues found for this scan. The assessment completed with zero findings."
        )
    ...
    def _finding_card(self, index: int, f: dict) -> QFrame:
        ...
        lay.addWidget(self._meta_row("Location", str(f.get("location", f.get("url", "")))))
    ...
    def _reset_view(self):
        self._last_report = None
        self.meta_label.setText("Generate a report after a successful scan.")
        for chip, val in (
            (self.chip_high, "0"),
            (self.chip_medium, "0"),
            (self.chip_low, "0"),
    ...
    def _render_report(self, report: dict):
        ...
        if not has_map:
            self.map_empty.setText(
                "No CWE/OWASP mappings — no issues were found in this assessment."
                if total == 0
                else "No CWE/OWASP mappings available."
            )
    ...
    def generate_report(self):
        ...
        if not SharedState.has_scan_data():
            self._reset_view()
            self.exec_text.setText(
                "No scan data available. Run a scan from Create Scan first."
            )
            return

        url = SharedState.current_url
        findings = SharedState.findings or []
        scan_type = getattr(SharedState, "scan_type", None) or "Dynamic"

        # ========== UNCOMMENT when connecting to Member 6 report module ==========
        # from reporting.report_generator import generate_report
        # report = generate_report(url, findings, scan_type=scan_type)
        # ========== UNCOMMENT end ==========

        # ========== DELETE when connecting to Member 6 report module ==========
        report = build_report_dict(url, findings, scan_type=scan_type)
        # ========== DELETE end ==========
        ...
        if main_window and hasattr(main_window, "show_toast"):
            ...
            if n == 0:
                ...
                main_window.show_toast("Report generated")
