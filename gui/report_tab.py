...
class ReportTab(QWidget):
    ...
    def _render_report(self, report: dict):
        ...
        summary = report.get("summary") or report.get("scan_summary") or {}
        ...
        for k, v in list(sans_map.items())[:6]:
            ...
            has_map = True
        scan_total = int((report.get("scan_summary") or {}).get("Total", 0) or 0)
        if not has_map:
            self.map_empty.setText(
    ...
    def generate_report(self):
        ...
        if main_window and hasattr(main_window, "show_toast"):
            n_scan = len(report.get("findings") or [])
            n_plat = len(report.get("stack_findings") or [])
    ...
    def export_pdf(self):
        ...
        try:
            ...
            saved_path = export_pdf(
                url=SharedState.current_url,
                findings=list(self._last_report.get("findings") or []),
                output_path=path,
