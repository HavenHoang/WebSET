from PyQt6.QtWidgets import (
    ...
    QScrollArea, QSizePolicy, QLayout, QLayoutItem
)
...
class _FlowLayout(QLayout):
    """Simple wrapping layout so chips never clip off the edge."""
...
class ReportTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Standards mapping
        self.map_card = self._section_card("Standards mapping")
        ...
        # Platform evaluation (Get Stack)
        self.platform_card = self._section_card("Platform evaluation (Get Stack)")
    ...
    def _render_report(self, report: dict):
        ...
        if not stack_findings:
            ...
            for i, f in enumerate(stack_findings, start=1):
                self.platform_layout.addWidget(self._finding_card(i, f))
    ...
    def export_pdf(self):
        ...
        if not has_scan and not has_stack:
            self._set_executive("No session data. Run a scan first.")
            return
