...
class TestResultCard(QFrame):
    ...
    def _fit_edit(self, box):
        ...
        box.document().setDocumentMargin(6)
        width = box.viewport().width()
        if width < 80:
            width = max(self.width() - 56, 420)
        box.document().setTextWidth(width)
        doc_h = int(box.document().size().height()) + 14
        lines = max(1, box.toPlainText().count("\n") + 1)
        floor = 40 if lines <= 2 else 52
        cap = 480
        h = min(max(floor, doc_h), cap)
