...
def _standards_cell(f: dict) -> str:
    """Visible multi-line standards (same layout as Alerts)."""
...
def _std_tip(f: dict) -> str:
    """Full standards + remediation for hover tooltip."""
    parts = [
...
class ScanDetailDialog(QDialog):
    ...
    def _build(self, scan: dict, findings: list, local_scan_no: int | None):
        ...
        tip = QLabel(
            "Historical findings for this scan · Standards: CWE / OWASP / NIST / SANS "
            "(same mapping as Alerts)"
        )
        tip.setStyleSheet("color:#64748b; font-size:11px; background: transparent;")
        tip.setWordWrap(True)
        root.addWidget(tip)

        # Same column model as Alerts tables
        ...
        if not findings:
            ...
            for i, f in enumerate(findings):
                ...
                table.setItem(i, 3, self._item(conf, color="#334155", center=True, tip=tip_text))

                loc = str(f.get("location") or f.get("url") or "")
                loc_item = self._item(_short_url(loc, 52), color="#475569", tip=loc or tip_text)
                ...
                rem_item = self._item(rem, color="#0f766e", tip=rem if rem != "—" else tip_text)
                rem_item.setFont(normal)
                table.setItem(i, 6, rem_item)

                # Row height for multi-line Standards + long description/remediation
                lines = max(
                    std_text.count("\n") + 1,
                    max(1, (len(desc) // 48) + 1),
                    max(1, (len(rem) // 48) + 1),
