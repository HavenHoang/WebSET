...
def _split_history_findings(findings: list) -> tuple[list, list]:
    rows = findings or []
    scan_findings = [
        f for f in rows if str(f.get("scan_origin") or "") != PLATFORM_ORIGIN
    ]
    platform_findings = [
        f for f in rows if str(f.get("scan_origin") or "") == PLATFORM_ORIGIN
    ]
    return scan_findings, platform_findings


def _load_tech_stacks_for_scan(scan_id: int) -> list:
    if not scan_id:
        return []
    try:
        from core.db import list_tech_stacks_for_scan
        return list(list_tech_stacks_for_scan(int(scan_id)) or [])
    except Exception:
        pass
    try:
        from core.db import get_tech_stacks_for_scan
        return list(get_tech_stacks_for_scan(int(scan_id)) or [])
    except Exception:
        return []
...
def export_saved_scan_pdf(parent, scan: dict, findings: list, local_scan_no=None) -> bool:
    scan = scan or {}
    findings = _enrich_findings(findings or [])
    scan_findings, platform_findings = _split_history_findings(findings)
    scan_id = scan.get("scan_id")
    stacks = _load_tech_stacks_for_scan(scan_id)
    url = scan.get("url") or ""
    scan_type = scan.get("scan_type") or "Dynamic"
    case_name = scan.get("case_name") or "Case"
    no_part = f"Scan{local_scan_no}" if local_scan_no else f"Scan{scan_id or ''}"
    safe_case = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in str(case_name))[:40]
    default_name = f"WebSET_{safe_case}_{no_part}.pdf"
    path, _ = QFileDialog.getSaveFileName(
        parent,
        "Export saved scan report",
        default_name,
        "PDF Files (*.pdf)",
    )
    if not path:
    ...
    try:
        ...
        report = generate_report(
            ...
            scan_type=scan_type,
            tech_stacks=stacks,
            stack_findings=platform_findings,
        )
        saved = export_pdf(
            url=url,
            findings=scan_findings,
        ...
        QMessageBox.warning(parent, "Export PDF", f"Could not export PDF:\n{e}")
        return False
    QMessageBox.information(parent, "Export PDF", f"PDF saved:\n{saved}")
    main = parent.window() if parent is not None else None
    if main is not None and hasattr(main, "show_toast"):
        main.show_toast("History report exported")
    return True
...
def _location_display(loc: str, max_len: int = 52) -> str:
    text = str(loc or "")
    if "\n" in text:
        return text
    return _short_url(text, max_len)
...
class ScanDetailDialog(QDialog):
    def __init__(self, scan: dict, findings: list, local_scan_no: int | None = None, parent=None):
        ...
        self._local_scan_no = local_scan_no
    ...
    def _badge(self, severity: str) -> QWidget:
        ...
        lab.setStyleSheet(
            ...
            f"font-weight:700; font-size:11px; padding:1px 8px;"
        )
        wrap = QWidget()
    ...
    def _style_table(self, table: QTableWidget, header_bg: str):
    ...
    def _make_stacks_table(self, stacks: list) -> QTableWidget:
        table = QTableWidget(0, 4)
        table.setHorizontalHeaderLabels(["NAME", "CATEGORY", "VERSION", "DESCRIPTION"])
        self._style_table(table, "#1b4f72")
        ...
        for i, tech in enumerate(stacks):
            table.insertRow(i)
            name = str(tech.get("name") or "")
            cat = str(tech.get("category") or "")
            ver = str(tech.get("version") or "") or "—"
            desc = str(tech.get("description") or "")
            n = self._item(name, bold=True)
            n.setFont(name_font)
            table.setItem(i, 0, n)
            c = self._item(cat, color="#475569")
            c.setFont(normal)
            table.setItem(i, 1, c)
            v = self._item(ver)
            v.setFont(normal)
            table.setItem(i, 2, v)
            d = self._item(desc, color="#334155", tip=desc)
            d.setFont(normal)
            table.setItem(i, 3, d)
    ...
    def _make_findings_table(
        self, findings: list, *, include_standards: bool = True
    ) -> QTableWidget:
        if include_standards:
            headers = [
                "Risk", "Vulnerability", "Standards",
                "Location", "Description", "Remediation",
            ]
            ncols = 6
            header_bg = "#1e3a5f"
        else:
            headers = ["Risk", "Finding", "Location", "Detail", "Remediation"]
            ncols = 5
            header_bg = "#0f766e"
        table = QTableWidget(0, ncols)
        table.setHorizontalHeaderLabels(headers)
        self._style_table(table, header_bg)
        hh = table.horizontalHeader()
        ...
        for i, f in enumerate(findings):
            table.insertRow(i)
            tip_text = _std_tip(f) if include_standards else _platform_tip(f)
            ...
            if include_standards:
                ...
                loc_item = self._item(
                    _location_display(loc, 52), color="#475569", tip=loc or tip_text
                )
                loc_item.setFont(normal)
                table.setItem(i, 3, loc_item)
            ...
            table.setRowHeight(i, max(64, 20 * lines + 12))
        _fit_table(table)
        return table
    ...
    def _build(self, scan: dict, findings: list, local_scan_no: int | None):
        ...
        scan_findings = _collapse_similar_findings(scan_findings)
        platform_findings = _collapse_similar_findings(platform_findings)
        stacks = _load_tech_stacks_for_scan(scan.get("scan_id"))
        ...
        for f in scan_findings + platform_findings:
            sev = str(f.get("severity", "Low"))
        ...
        for key, color in (
            ...
            chip.setMinimumWidth(70 if key in ("Medium", "Total") else 62)
        ...
        sum_row.addStretch()
        note = QLabel(
            f"Start Scan: {len(scan_findings)} · Platform: {len(platform_findings)} "
            f"· Stacks: {len(stacks)}"
        )
        ...
        root.addWidget(self._section_label("Detected tech stacks"))
        if stacks:
            root.addWidget(self._make_stacks_table(stacks))
        else:
            root.addWidget(
                self._make_empty_card(
                    "No tech stacks recorded",
                    "Run Get Stack from Create Scan to store detected technologies.",
                )
            )

        root.addWidget(self._section_label("Scan findings (Start Scan)"))
        if scan_findings:
            root.addWidget(self._make_findings_table(scan_findings, include_standards=True))
        else:
            root.addWidget(
                self._make_empty_card(
        ...
        root.addWidget(self._section_label("Platform evaluation (Get Stack)"))
        if platform_findings:
            root.addWidget(
                self._make_findings_table(platform_findings, include_standards=False)
            )
        else:
            root.addWidget(
                self._make_empty_card(
        ...
        footer = QFrame()
        footer.setStyleSheet("QFrame { background: #f1f5f9; border-top: 1px solid #e2e8f0; }")
        btn_row = QHBoxLayout(footer)
        btn_row.setContentsMargins(18, 10, 18, 12)
        btn_row.addStretch()
    ...
    def _export_pdf(self):
        export_saved_scan_pdf(
            self, self._scan, self._findings, local_scan_no=self._local_scan_no
        )
...
class CasesPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.cases_table.setTextElideMode(Qt.TextElideMode.ElideRight)
        self.cases_table.setIconSize(QSize(28, 28))
        ...
        self.scan_tip = QLabel(
            "Select a scan then Export PDF, or double-click to open detail. "
        )
        ...
        self.export_scan_btn = QPushButton("Export PDF")
        self.export_scan_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.export_scan_btn.setStyleSheet("""
            QPushButton {
            ...
            QPushButton:hover { background: #15803d; }
        """)
        self.export_scan_btn.clicked.connect(self.export_selected_scan_pdf)
        scan_actions.addWidget(self.export_scan_btn)
        sp_l.addLayout(scan_actions)
    ...
    def refresh(self):
        ...
        for i, row in enumerate(self._cases):
            ...
            full_name = str(row.get("case_name") or "Untitled")
            name_item = QTableWidgetItem(full_name)
            name_item.setIcon(self._make_avatar(full_name[:1] if full_name else "S"))
    ...
    def _load_scans_for_case(self, case_id: int):
        ...
        for i, row in enumerate(scans):
            ...
            prog_item = QTableWidgetItem(f"{pct}%")
            prog_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            prog_item.setForeground(QColor("#94a3b8"))
            self.scans_table.setItem(i, 3, prog_item)
    ...
    def _on_scan_selected(self):
        rows = self.scans_table.selectionModel().selectedRows()
        if not rows:
            self._selected_scan_id = None
            return
        item = self.scans_table.item(rows[0].row(), 0)
        if not item:
            self._selected_scan_id = None
            return
        sid = item.data(Qt.ItemDataRole.UserRole)
        self._selected_scan_id = int(sid) if sid is not None else None
    ...
    def export_selected_scan_pdf(self):
        if self._selected_scan_id is None:
            QMessageBox.information(
                self,
                "Export PDF",
                "Select a scan row first, or double-click to open detail and export there.",
            )
            return
        try:
            from core.db import get_scan_by_id, list_findings_for_scan
            scan = get_scan_by_id(self._selected_scan_id)
            findings = list_findings_for_scan(self._selected_scan_id)
        except Exception as e:
            QMessageBox.warning(self, "Export PDF", f"Could not load scan: {e}")
            return
        if not scan:
            QMessageBox.information(self, "Export PDF", "Scan not found.")
            return
        local_no = self._scan_local_nos.get(int(self._selected_scan_id))
        export_saved_scan_pdf(self, scan, findings, local_scan_no=local_no)
