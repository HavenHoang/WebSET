...
def _standards_cell(f: dict) -> str:
    """Start Scan only — CWE / OWASP / NIST / SANS."""
...
def _std_tip(f: dict) -> str:
    """Tooltip for Start Scan findings (includes standards)."""
    parts = [
...
def _platform_tip(f: dict) -> str:
    """Tooltip for Platform notes — no standards mapping."""
    parts = [
...
class ScanDetailDialog(QDialog):
    ...
    def _make_findings_table(
        ...
        """
        Start Scan → include_standards=True (CWE/OWASP/NIST/SANS).
        Platform evaluation → include_standards=False (guidance only + confidence).
        """
        if include_standards:
            headers = [
                "Risk", "Vulnerability", "Standards", "Conf.",
        ...
        for i, f in enumerate(findings):
            ...
            if include_standards:
                ...
                conf = str(f.get("confidence") or "—")
                table.setItem(
                    i, 2, self._item(conf, color="#334155", center=True, tip=tip_text)
                )
    ...
    def _build(self, scan: dict, findings: list, local_scan_no: int | None):
        ...
        tip = QLabel(
            "Historical record · Start Scan findings may include CWE / OWASP / NIST / SANS. "
            "Platform evaluation is guidance only (no standards mapping)."
        )
        ...
        if scan_findings:
            root.addWidget(
                self._make_findings_table(scan_findings, include_standards=True), 1
            )
        else:
            root.addWidget(
        ...
        if platform_findings:
            root.addWidget(
                self._make_findings_table(platform_findings, include_standards=False), 1
            )
        else:
            root.addWidget(
                self._make_empty_card(
...
class CasesPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.scan_tip = QLabel(
            ...
            "FINDINGS = Start Scan · PLATFORM = Get Stack notes (no standards mapping)."
        )
