from PyQt6.QtWidgets import (
    ...
    QStyleFactory, QListView, QDialog, QAbstractItemView
)
...
class ScanDetailDialog(QDialog):
    ...
    def _item(self, text: str, *, bold=False, color="#0f172a", center=False, tip=None):
        """Table cell with explicit dark text (never inherits light-on-light)."""
    ...
    def _build(self, scan: dict, findings: list, local_scan_no: int | None):
        ...
        tip = QLabel("Findings stored for this historical scan only")
        tip.setStyleSheet("color:#64748b; font-size:11px; background: transparent;")
        root.addWidget(tip)

        table = QTableWidget(0, 6)
        table.setHorizontalHeaderLabels(
            ["Risk", "Vulnerability", "CWE", "Conf.", "Location", "Description"]
        )
        ...
        if not findings:
            table.setRowCount(1)
            empty = self._item("No findings stored for this scan.", color="#64748b")
            ...
            for i, f in enumerate(findings):
                ...
                table.setItem(i, 1, self._item(name, bold=True, color="#0f172a"))

                cwe = str(f.get("cwe_id") or f.get("cweId") or "—")
                tips = []
                if f.get("wasc_id"):
                    tips.append(f"WASC: {f.get('wasc_id')}")
                if f.get("owasp") or f.get("owasp_tags"):
                    tips.append(f"OWASP: {f.get('owasp') or f.get('owasp_tags')}")
                if f.get("plugin_id"):
                    tips.append(f"Plugin: {f.get('plugin_id')}")
                table.setItem(
                    i,
                    2,
                    self._item(
                        ...
                        color="#1d4ed8",
                        center=True,
                        tip=" · ".join(str(x) for x in tips) if tips else None,
                    ),
                )

                conf = str(f.get("confidence") or "—")
                table.setItem(i, 3, self._item(conf, color="#334155", center=True))

                loc = str(f.get("location") or f.get("url") or "")
                table.setItem(
                    i,
                    4,
                    self._item(_short_url(loc, 48), color="#475569", tip=loc),
                )

                desc = str(f.get("description") or "")
                table.setItem(
                    i, 5, self._item(desc, color="#334155", tip=desc)
                )
...
class CasesPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.scan_tip = QLabel(
            "Double-click a scan row to open the detailed findings report."
        )
        ...
        self.scans_table.setHorizontalHeaderLabels(
            ["SCAN", "TYPE", "STATUS", "PROGRESS", "FINDINGS"]
        )
