...
def _merge_findings(existing: list, new_items: list) -> list:
    ...
    def key(f):
        return (
            str(f.get("vulnerability", "")).lower(),
            str(f.get("location", "")).lower(),
            str(f.get("plugin_id", "")).lower(),
        )

    index = {key(f): i for i, f in enumerate(existing)}
...
class CreateScanPage(QWidget):
    ...
    def init_ui(self):
        ...
        subtitle = QLabel(
            ...
            "Get Stack detects tech stacks and platform findings "
            "(merged with Start Scan findings when the same target is used). "
            "After Start Scan, Get Stack also updates that scan’s history in Cases."
        )
        ...
        zip_note = QLabel(
            "Static: Get Stack (ZIP) = stacks + platform findings. "
            "Start Static Scan = backend analysis. Findings merge when the same ZIP path is used. "
            "After Static Scan, Get Stack (ZIP) updates that scan’s history in Cases."
        )
    ...
    def _fail_dynamic(self, message: str):
        """Stop progress and show error — never treat as success."""
    ...
    def _refresh_related_pages(self, include_alerts: bool = True):
        main_window = self.window()
        if not main_window:
            return
        if hasattr(main_window, "tech_stack_page"):
    ...
    def _persist_merged_scan_to_history(self) -> bool:
        try:
            from core.shared_state import SharedState
            ...
            print("persist merged scan error:", e)
            return False
    ...
    def clear_form(self):
        ...
        self._refresh_related_pages(include_alerts=True)
    ...
    def get_stack(self):
        ...
        try:
            ...
            findings = _enrich(result.get("findings") or [])
            SharedState.tech_stacks = stacks
            SharedState.case_name = self.app_input.text().strip() or "Sample App"
            prev_url = getattr(SharedState, "current_url", None)
            prev = list(getattr(SharedState, "findings", None) or [])
            if _same_target(prev_url, url) and prev:
                SharedState.findings = _merge_findings(prev, findings)
            else:
            ...
            n_s, n_f = len(stacks or []), len(findings or [])
            n_total = len(SharedState.findings or [])
            hist_ok = self._persist_merged_scan_to_history()
            if hist_ok:
                status = (
                    f"Tech stack (URL): {n_s} tech(s), {n_f} platform finding(s) "
                    f"(session total {n_total}). History updated for this scan."
                )
            elif getattr(SharedState, "scan_id", None) is None:
                status = (
                    f"Tech stack (URL): {n_s} tech(s), {n_f} platform finding(s) "
                    f"(session total {n_total}). Run Start Scan first to save history."
                )
            else:
                status = (
                    f"Tech stack (URL): {n_s} tech(s), {n_f} platform finding(s) "
                    f"(session total {n_total}). History save skipped — check db helper."
                )
            self.update_status(status)
            self._toast(f"Stack: {n_s} · +{n_f} findings")
            self._refresh_related_pages(include_alerts=True)
        except Exception as e:
    ...
    def get_stack_static(self):
        ...
        try:
            ...
            prev_url = getattr(SharedState, "current_url", None)
            prev = list(getattr(SharedState, "findings", None) or [])
            if _same_target(prev_url, target) and prev:
                SharedState.findings = _merge_findings(prev, findings)
            else:
                SharedState.findings = findings
            ...
            if hist_ok:
                ...
                status = (
                    f"Tech stack (ZIP): {n_s} tech(s), {n_f} platform finding(s) "
                    f"(session total {n_total}). Run Start Static Scan first to save history."
                )
            else:
                status = (
                    f"Tech stack (ZIP): {n_s} tech(s), {n_f} platform finding(s) "
            ...
            self.update_status(status)
            self._toast(f"Stack (ZIP): {n_s} · +{n_f} findings")
            self._refresh_related_pages(include_alerts=True)
        except Exception as e:
    ...
    def _do_dynamic_scan(self):
        ...
        try:
            ...
            # Compulsory: error dict → never start progress / never save success
            if isinstance(result, dict) and result.get("error"):
    ...
    def _save_and_finish_dynamic(self, gen: int):
        # Guard: never show success after an error
        if gen != self._scan_gen or self._scan_failed:
            self.scan_button.setEnabled(True)
            return
        ...
        try:
            ...
            incoming = _enrich(self._findings)
            prev_url = getattr(SharedState, "current_url", None)
            prev = list(getattr(SharedState, "findings", None) or [])
            if _same_target(prev_url, self._current_scan_url) and prev:
                SharedState.findings = _merge_findings(prev, incoming)
            else:
                SharedState.findings = incoming
    ...
    def _save_and_finish_static(self):
        """KEEP after integration — merge + DB + refresh."""
        session_n = 0
        try:
            from core.shared_state import SharedState
            from core.db import save_full_scan
