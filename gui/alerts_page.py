...
_ACTIVE_TYPES = frozenset({"xss", "sqli", "nosqli", "xxe", "path_traversal", "command_injection", "idor"})
...
def _infer_vuln_type(f: dict) -> str:
    ...
    plugin = str(f.get("plugin_id") or "").lower()
    if v == "idor" or plugin == "idor-bola" or "insecure direct" in name or "broken object" in name:
        return "idor"
    if v == "xxe" or "external entity" in name or "xxe" in name:
        return "xxe"
...
def _finding_url(f: dict) -> str:
    ...
    if loc and "\n" not in loc:
        return loc
    return str(f.get("url") or f.get("endpoint") or loc.split("\n")[0] if loc else "").strip()


def _finding_param(f: dict) -> str:
    return str(f.get("param") or f.get("input") or f.get("parameter") or "").strip()


def _target_from_finding(f: dict) -> dict:
    url = _finding_url(f)
    param = _finding_param(f)
    return {
        "url": url,
        "endpoint": url,
        "method": str(f.get("method") or "GET").upper() or "GET",
        "param": param,
        "input": param,
        "param_location": str(
            f.get("param_location") or f.get("input_location") or "query"
        ),
...
def _is_active_testable(f: dict) -> bool:
    ...
    if vtype not in _ACTIVE_TYPES:
        return False
    targets = f.get("active_test_targets") or []
    if targets:
        for t in targets:
            url = str((t or {}).get("url") or (t or {}).get("endpoint") or "").strip()
            if not url:
                continue
            if vtype in {"xss", "sqli"}:
                param = str((t or {}).get("param") or (t or {}).get("input") or "").strip()
                if param:
                    return True
            else:
                return True
    ...
    if not url:
        return False
    if vtype in {"xss", "sqli"} and not _finding_param(f):
        return False
    return True
...
def _collapse_similar_findings(findings: list) -> list:
    ...
    for f in findings or []:
        ...
        target = _target_from_finding(f)
        if key not in groups:
            item = dict(f)
            item["_locs"] = [loc] if loc else []
            item["_targets"] = [target] if target.get("url") else []
            groups[key] = item
            order.append(key)
            continue
        if loc and loc not in groups[key]["_locs"]:
        ...
        if target.get("url"):
            seen = {
                ...
                for t in groups[key]["_targets"]
            }
            sig = (
                str(target.get("url") or ""),
                str(target.get("param") or ""),
                str(target.get("method") or ""),
            )
            if sig not in seen:
                groups[key]["_targets"].append(target)
    out = []
    for key in order:
        item = groups[key]
        locs = item.pop("_locs", [])
        targets = item.pop("_targets", [])
        if locs:
            item["location"] = "\n".join(locs)
        item["active_test_targets"] = targets
        if targets:
            item["url"] = targets[0].get("url") or item.get("url")
            item["endpoint"] = item["url"]
            item["param"] = targets[0].get("param") or item.get("param")
            item["method"] = targets[0].get("method") or item.get("method") or "GET"
        out.append(item)
    return out
...
def _std_tip(f: dict) -> str:
    ...
    targets = f.get("active_test_targets") or []
    if len(targets) > 1:
        parts.append(f"Active Test paths: {len(targets)} (choose one on Payload)")
...
def _normalize_finding_for_active_test(f: dict) -> dict:
    out = dict(f or {})
    targets = list(out.get("active_test_targets") or [])
    if not targets:
        t = _target_from_finding(out)
        if t.get("url"):
            targets = [t]
    out["active_test_targets"] = targets
    first = targets[0] if targets else {}
    url = first.get("url") or out.get("url") or out.get("endpoint") or ""
    param = first.get("param") or _finding_param(out)
    out["url"] = url
    out["endpoint"] = url
    out["method"] = first.get("method") or out.get("method") or "GET"
    out["param"] = param
    out["input"] = param
    out["param_location"] = (
        first.get("param_location")
        or out.get("param_location")
        or out.get("input_location")
        or "query"
    )
...
class AlertsPage(QWidget):
    ...
    def _active_test_button(self, finding: dict) -> QWidget:
        ...
        n = len(finding.get("active_test_targets") or [])
        if n > 1:
            btn.setToolTip(f"Choose 1 of {n} paths on Payload, then run one probe.")
        else:
            btn.setToolTip(
    ...
    def _on_active_test(self, finding: dict):
        ...
        if payload_page is None and main is not None:
            payload_page = getattr(main, "payload_tab", None)
        ...
        if main is not None and hasattr(main, "show_toast"):
            n = len(target.get("active_test_targets") or [])
            name = str(target.get("vulnerability") or target.get("name") or "Finding")
            if n > 1:
                main.show_toast(f"Active Test — pick 1 of {n} paths, then Run")
            else:
    ...
    def _fill_table(
        ...
        for i, f in enumerate(findings[:40]):
            ...
            if _is_active_testable(f):
                table.setCellWidget(i, 6, self._active_test_button(f))
            else:
                table.setCellWidget(i, 6, self._passive_action_placeholder(f))
    ...
    def refresh(self):
        ...
        dyn_rows = _collapse_similar_findings(dyn_rows)
        st_rows = _collapse_similar_findings(st_rows)
