...
ACTIVE_TYPES = frozenset({"xss", "sqli", "nosqli", "path_traversal", "command_injection"})
...
def _execute_one(finding: dict, test: dict, request_text: str = "") -> dict:
    ...
    if not _is_active_testable(finding):
        ...
        out["text"] = (
            ...
            "(xss / sqli / nosqli / path_traversal / command_injection) with URL + param."
        )
        return out
    try:
    ...
    if allowed and not in_scope(scope_url, allowed):
        ...
        return out
    if _finding_vtype(finding) == "nosqli" and "authorization" not in {k.lower() for k in headers}:
