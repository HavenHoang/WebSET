    ...
    "idor": [
        {
            "id": "idor_1_unauth",
            "step": 1,
            "label": "IDOR — unauthenticated collection GET",
            "marker": "",
            "expect": "idor_denied",
            "auth": False,
            "hint": (
                "GET the collection with no Authorization header. "
                "A 401/403 means the route is gated. A user dump here is "
        ...
        },
        {
            "id": "idor_2_auth",
            "step": 2,
            "label": "IDOR — authenticated collection GET",
            "marker": "",
            "expect": "idor_dump",
            "auth": True,
            "hint": (
                "Replay the same GET with a Bearer token from a normal session. "
                "Confirmation = JSON with more than one user identity."
            ),
        },
    ],
}
