...
_AUTH_FAIL_RE = re.compile(
    r"(invalid (user(name)?|e-?mail|password|credential|login|account)|"
    r"(username|email|password) (is )?incorrect|"
    r"incorrect (user(name)?|password|credential)|"
    r"login failed|authentication failed|access denied|"
    r"username and/?or password|"
    r"wrong password|bad credentials)",
    re.I,
)
_AUTH_OK_RE = re.compile(
    r"(welcome to the password protected|"
    r"you (have )?successfully logged in|"
    r"you (have )?logged in|"
    r"you are now logged|"
    r"logged[- ]in as\b|"
    r"signed[- ]in as\b|"
    r"login successful|"
    r"authentication successful|"
    r"successfully (logged|signed) in|"
...
def _auth_success(status_code: int, body: str) -> bool:
    ...
    if isinstance(err, str) and _AUTH_FAIL_RE.search(err):
        return False
    if str(data.get("status") or "").lower() in ("error", "fail", "failed"):
        if not any(
            (data.get(k) for k in ("token", "access_token", "accessToken", "jwt"))
        ):
            return False
...
def analyse_sqli(marker: str, body: str, status_code: int, expect: str = "") -> dict:
    ...
    if expect == "auth_success":
        if auth:
            return _base_result(
                ...
                detail=(
                    "Login-like response contains a session token, user object, "
                    "or a logged-in page after a SQL tautology. The password "
        ...
        return _base_result(
            ...
            confirmation="NOT CONFIRMED",
            conclusion=(
                "SQL tautology broke the query — login did not succeed"
                if db_error else
                "Authentication bypass not confirmed"
            ),
            detail=(
                "The authentication probe produced a database / ORM error. "
                "That shows the field reaches SQL, but this step is only "
                "confirmed when the response is a session token, user object, "
                "or a logged-in page — not an error page."
                if db_error else
    ...
    if auth and expect in ("boolean_or_auth", ""):
        return _base_result(
            found_in_body=found,
            db_error_signal=db_error,
            auth_success=True,
...
def analyse_idor(marker: str, body: str, status: int, expect: str = "") -> dict:
    dump, evidence = _idor_user_dump(body)
    ...
    if expect == "idor_dump":
        if dump:
            return {
                ...
                "auth_success": False,
                "confirmation": "CONFIRMED",
                "conclusion": "CONFIRMED — collection returned other users",
                "detail": "JSON listed more than one user identity for this session.",
                "expect": expect,
                "evidence": evidence or f"HTTP {code}",
            }
        return {
            "found_in_body": False,
            "encoded": False,
            ...
            "auth_success": False,
            "confirmation": "NOT CONFIRMED",
            "conclusion": "NOT CONFIRMED — authenticated GET did not dump users",
            "detail": f"HTTP {code}",
            "expect": expect,
    ...
    if dump:
        return {
            ...
            "auth_success": False,
            "confirmation": "CONFIRMED",
            "conclusion": "CONFIRMED — collection leaked users without a bearer token",
            "detail": "The unauthenticated GET already returned multiple user identities.",
            "expect": expect,
            "evidence": evidence or f"HTTP {code}",
        }
    if code in (401, 403):
        return {
            ...
            "auth_success": False,
            "confirmation": "LIKELY",
            "conclusion": "LIKELY — collection requires authentication",
            "detail": "Unauthenticated GET was denied. Run the bearer step next.",
            "expect": expect,
            "evidence": f"HTTP {code}",
        }
    return {
        ...
        "auth_success": False,
        "confirmation": "NOT CONFIRMED",
        "conclusion": "NOT CONFIRMED — no user-collection dump",
        "detail": f"HTTP {code}",
        "expect": expect,
        "evidence": f"HTTP {code}",
    }
...
def analyse_for_vuln_type(
    ...
    if v in ("idor", "bola"):
        return analyse_idor(marker, body, status, expect=expect)
    if v == "path_traversal":
