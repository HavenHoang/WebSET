...
import time
from crawl.scope import normalise_url, is_http_url
from crawl.headers_cookies import headers_to_dict, set_cookie_list
...
def fetch_target(url: str, *, timeout: float = DEFAULT_TIMEOUT,
    ...
    if use_browser:
        try:
            from crawl.browser import fetch_with_selenium
...
def _fail(url: str, error: str) -> dict:
    return {"ok": False, "url": url or "", "status": 0, "headers": {},
            "set_cookie": [], "body": "", "elapsed_ms": 0, "error": error}
