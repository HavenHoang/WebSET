...
def register_user(username: str, password: str, display_name: str | None = None) -> dict:
    """
    Create a new user. Returns {"id", "username", "display_name"}.
    Raises ValueError on validation / duplicate username.
    """
...
def authenticate_user(username: str, password: str) -> dict | None:
    ...
    Returns {"id", "username", "display_name"} or None.
    """
    username = (username or "").strip()
...
def update_scan_findings_and_stacks(
    ...
    """
    Replace findings (and optionally tech stacks) for an existing scan.
    Used after Get Stack merges platform findings into the current Start Scan session.
    tech_stacks is keyed by scan_id (same as save_tech_stacks).
    """
    if scan_id is None:
        return False
    init_db()
    ...
    try:
        ...
        cur.execute("DELETE FROM findings WHERE scan_id = ?", (sid,))
    ...
    # Reuse existing writers (includes enrich markers / owasp_tags / nist / sans)
...
def get_scan_by_id(scan_id: int) -> dict | None:
    """One scan + case name (for historical detail dialog)."""
    init_db()
    conn = _connect()
    cur = conn.cursor()
    cur.execute(
...
def list_findings_for_scan(scan_id: int) -> list:
    """Findings for one historical scan (enriched for GUI detail view)."""
...
def get_dashboard_stats(user_id: int | None = None) -> dict:
    ...
    if user_id is not None:
        cur.execute(
            """
            SELECT f.cwe_id, COUNT(*) AS n FROM findings f
            JOIN scans s ON s.id = f.scan_id
            JOIN cases c ON c.id = s.case_id
            WHERE c.user_id = ? AND f.cwe_id IS NOT NULL AND f.cwe_id != ''
            GROUP BY f.cwe_id
            """,
            (user_id,),
        )
        ...
        cur.execute(
            """
            SELECT cwe_id, COUNT(*) AS n FROM findings
            WHERE cwe_id IS NOT NULL AND cwe_id != ''
            GROUP BY cwe_id
            """
        )
    for r in cur.fetchall():
        cwe_counts[str(r["cwe_id"])] = int(r["n"])

    nist_counts = {}
    if user_id is not None:
        cur.execute(
            """
            SELECT f.nist, COUNT(*) AS n FROM findings f
            JOIN scans s ON s.id = f.scan_id
            JOIN cases c ON c.id = s.case_id
            WHERE c.user_id = ? AND f.nist IS NOT NULL AND f.nist != ''
            GROUP BY f.nist
            """,
            (user_id,),
        )
        ...
        cur.execute(
            """
            SELECT nist, COUNT(*) AS n FROM findings
            WHERE nist IS NOT NULL AND nist != ''
            GROUP BY nist
            """
        )
    for r in cur.fetchall():
        nist_counts[str(r["nist"])] = int(r["n"])

    sans_counts = {}
    if user_id is not None:
        cur.execute(
            """
            SELECT f.sans, COUNT(*) AS n FROM findings f
            JOIN scans s ON s.id = f.scan_id
            JOIN cases c ON c.id = s.case_id
            WHERE c.user_id = ? AND f.sans IS NOT NULL AND f.sans != ''
            GROUP BY f.sans
            """,
            (user_id,),
        )
        ...
        cur.execute(
            """
            SELECT sans, COUNT(*) AS n FROM findings
            WHERE sans IS NOT NULL AND sans != ''
            GROUP BY sans
            """
        )
    for r in cur.fetchall():
        sans_counts[str(r["sans"])] = int(r["n"])
