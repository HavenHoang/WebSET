...
def _migrate_findings_columns(cur):
    ...
    Includes standards columns and Active Test target fields.
    """
    ...
    # Active Test target fields (Start Scan findings)
...
def _finding_row_to_dict(r) -> dict:
    ...
    # Active Test fields (empty string if missing / Platform row)
...
def save_findings(scan_id: int, findings: list, scan_origin: str = "Dynamic"):
    """
    Persist findings.
    Start Scan rows may be enriched with CWE/OWASP/NIST/SANS and Active Test fields.
    Platform rows (scan_origin == Platform) are stored as guidance only —
    no standards enrichment; Active Test fields left empty.
    """
    if not findings:
        return
    ...
    for raw in findings:
        ...
        if origin == PLATFORM_ORIGIN:
            # Platform evaluation — no standards mapping, no Active Test target
            f = dict(raw or {})
...
def save_full_scan(
    ...
    """
    Create case + scan row.
    - findings: Start Scan results (may be empty for Get-Stack-only history)
    - tech_stacks / stack_findings: optional Get Stack data (Platform origin)
    """
    if user_id is None:
...
def list_findings_for_scan(scan_id: int) -> list:
    ...
    try:
        from core.cwe_map import enrich_findings
        # enrich_finding skips Platform origin (no standards injected)
...
def get_dashboard_stats(user_id: int | None = None) -> dict:
    """
    Aggregate stats. CWE / OWASP / NIST / SANS counts exclude Platform notes
    (standards mapping applies to Start Scan findings only).
    """
    init_db()
    conn = _connect()
    cur = conn.cursor()
    ...
    # Standards counts — Start Scan only (exclude Platform)
    owasp_counts = {}
    if user_id is not None:
        cur.execute(
            """
