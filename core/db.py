    ...
    for raw in findings:
        ...
        cur.execute(
            ...
            INSERT INTO findings (
                scan_id, severity, vulnerability, location, description,
                confidence, cwe_id, wasc_id, owasp_tags, plugin_id, message_id, scan_origin
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                scan_id,
...
def get_findings(scan_id: int) -> list:
    ...
    cur.execute(
        """
        SELECT severity, vulnerability, location, description,
               confidence, cwe_id, wasc_id, owasp_tags, plugin_id, message_id, scan_origin
...
def list_findings_by_origin(scan_origin: str | None = None, limit: int = 100) -> list:
    ...
    if scan_origin:
        cur.execute(
            """
            SELECT
                f.severity, f.vulnerability, f.location, f.description,
                f.confidence, f.cwe_id, f.wasc_id, f.owasp_tags,
...
def get_dashboard_stats(user_id: int | None = None) -> dict:
    ...
    if user_id is not None:
        cur.execute(
            """
            SELECT f.severity, f.vulnerability, f.location, f.description,
                   f.confidence, f.cwe_id, f.wasc_id, f.owasp_tags, f.scan_origin
            FROM findings f
            JOIN scans s ON s.id = f.scan_id
            JOIN cases c ON c.id = s.case_id
            WHERE c.user_id = ?
        ...
        cur.execute(
            """
            SELECT severity, vulnerability, location, description,
                   confidence, cwe_id, wasc_id, owasp_tags, scan_origin
