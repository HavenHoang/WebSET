...
def _migrate_findings_columns(cur):
    ...
    if "cwe_id" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN cwe_id TEXT")
    if "wasc_id" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN wasc_id TEXT")
    if "owasp_tags" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN owasp_tags TEXT")
    if "nist" not in cols:
    ...
    if "plugin_id" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN plugin_id TEXT")
    if "message_id" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN message_id TEXT")
    if "scan_origin" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN scan_origin TEXT DEFAULT 'Dynamic'")
    if "method" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN method TEXT")
    if "param" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN param TEXT")
    if "param_location" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN param_location TEXT")
    if "context" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN context TEXT")
    if "vuln_type" not in cols:
        alters.append("ALTER TABLE findings ADD COLUMN vuln_type TEXT")
    for sql in alters:
        try:
            cur.execute(sql)
    ...
    if "confidence" in cols:
        try:
            cur.execute("ALTER TABLE findings DROP COLUMN confidence")
        except sqlite3.OperationalError:
            pass
...
def _finding_row_to_dict(r) -> dict:
    ...
    location = d.get("location") or d.get("url") or ""
    d["location"] = location
    d["url"] = location
    d["endpoint"] = location
    ...
    d["method"] = d.get("method") or ""
    param = d.get("param") or ""
    d["param"] = param
    d["input"] = param
    d["param_location"] = d.get("param_location") or ""
    d["context"] = d.get("context") or ""
    d["vuln_type"] = d.get("vuln_type") or ""
    return d
...
def save_findings(scan_id: int, findings: list, scan_origin: str = "Dynamic"):
    ...
    cur = conn.cursor()
    for raw in findings:
        origin = str((raw or {}).get("scan_origin") or scan_origin or "Dynamic")
        if origin == PLATFORM_ORIGIN:
            f = dict(raw or {})
            f["scan_origin"] = PLATFORM_ORIGIN
            owasp_s = "[]"
            cwe_val = ""
            wasc_val = ""
            nist_val = ""
            ...
            f = dict(raw or {})
            origin = str(f.get("scan_origin") or scan_origin or "Dynamic")
            ...
            cwe_val = str(f.get("cwe_id", f.get("cweId", "") or ""))
            wasc_val = str(f.get("wasc_id", f.get("wascId", "") or ""))
            nist_val = str(f.get("nist", f.get("nist_id", "") or ""))
            sans_val = str(f.get("sans", f.get("sans_id", "") or ""))
            method_val = str(f.get("method") or "")
            param_val = str(
                f.get("param") or f.get("input") or f.get("parameter") or ""
            )
            param_loc_val = str(
                f.get("param_location") or f.get("input_location") or ""
            )
            context_val = str(f.get("context") or "")
            vuln_type_val = str(
                f.get("vuln_type") or f.get("vulnerability_type") or ""
            )
        location_val = f.get("location") or f.get("url") or f.get("endpoint") or ""
        cur.execute(
            """
            INSERT INTO findings (
                scan_id, severity, vulnerability, location, description, remediation,
                cwe_id, wasc_id, owasp_tags, nist, sans,
                plugin_id, message_id, scan_origin,
                method, param, param_location, context, vuln_type
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                scan_id,
...
def save_full_scan(
    ...
    if stack_findings:
        ...
        for f in stack_findings:
            ...
            tagged.append(d)
        save_findings(scan_id, tagged, scan_origin=PLATFORM_ORIGIN)
    return {
        "case_id": case_id,
        "scan_id": scan_id,
        "user_id": int(user_id),
...
def list_findings_for_scan(scan_id: int) -> list:
    return get_findings(int(scan_id))
    ...
    if scan_origin:
        cur.execute(
            ...
            SELECT
                ...
                f.cwe_id, f.wasc_id, f.owasp_tags, f.nist, f.sans,
                f.plugin_id, f.message_id, f.scan_origin,
                f.method, f.param, f.param_location, f.context, f.vuln_type,
...
def get_platform_note_counts(user_id: int | None = None) -> list:
    ...
    if user_id is not None:
        cur.execute(
            """
            SELECT f.vulnerability AS vulnerability,
            ...
            WHERE c.user_id = ?
              AND f.scan_origin = ?
            GROUP BY f.vulnerability, f.severity
            ORDER BY count DESC
            """,
        ...
        cur.execute(
            """
            SELECT vulnerability, severity, COUNT(*) AS count
            FROM findings
            WHERE scan_origin = ?
...
def get_dashboard_stats(user_id: int | None = None) -> dict:
    ...
    if user_id is not None:
        cur.execute(
            ...
            SELECT f.severity, f.vulnerability, f.location, f.description, f.remediation,
                   f.cwe_id, f.wasc_id, f.owasp_tags, f.nist, f.sans, f.scan_origin,
                   f.method, f.param, f.param_location, f.context, f.vuln_type
            FROM findings f
            JOIN scans s ON s.id = f.scan_id
            JOIN cases c ON c.id = s.case_id
        ...
        cur.execute(
            """
            SELECT severity, vulnerability, location, description, remediation,
                   cwe_id, wasc_id, owasp_tags, nist, sans, scan_origin,
                   method, param, param_location, context, vuln_type
    ...
    static_recent_findings = [_finding_row_to_dict(r) for r in cur.fetchall()]
    conn.close()
    return {
        "cases": cases,
        "scans": scans,
        ...
        "static_recent_findings": static_recent_findings,
        "platform_note_counts": get_platform_note_counts(user_id),
