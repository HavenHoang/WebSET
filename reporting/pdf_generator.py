...
def _styles():
    ...
    return {
        ...
        # Numbered section heading -- also drives the Table of Contents.
        ...
        # Risk-card typography (kept separate so the number never collides
        # with the label -- each lives on its own Paragraph/row).
...
# ============================================================
# Numbered canvas -- adds "Page X of Y" once the full document is known
# ============================================================

class _NumberedCanvas(pdfcanvas.Canvas):
    """Buffers pages so the footer can show 'Page X of Y' on content pages."""
...
def _draw_geometric_background(canvas, closing=False):
    """
    Draw a premium WebSET cybersecurity cover/end-page background.
    All scan-specific text is added separately by ReportLab.

    The end page reuses this exact composition mirrored left-to-right, so
    the two bookend pages feel intentionally paired rather than duplicated.
    """

    canvas.saveState()

    if closing:
        # Mirror the whole composition horizontally for a bookend effect.
    ...
    # --------------------------------------------------------
    # Single diagonal seam between the navy hero and the white
    # foot of the page -- one clean cut, not a chevron. Tilted
    # around the hero's real bottom edge so it never drifts down
    # into the cover copy or metadata panel below it.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Top-right corner accent -- two nested, parallel triangles
    # (same slope, no crossing lines) with a single crisp
    # highlight stroke running along their shared seam.
    # --------------------------------------------------------

    outer_a, outer_b = 92 * mm, 70 * mm   # reach along top edge / right edge
    inner_a, inner_b = 50 * mm, 38 * mm   # same ratio -> parallel hypotenuse
    ...
    # --------------------------------------------------------
    # Understated corner accents in the white foot, kept clear
    # of the seam so nothing crosses or collides.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Thin double-rule frame around the whole page, offset in
    # from the trim edge -- gives the cover/end pages a bound,
    # finished, "printed dossier" feel.
    # --------------------------------------------------------
...
def _content_page(canvas, doc):
    """Marks the page so the numbered-canvas footer is drawn with 'Page X of Y',
    and draws a slim running header identifying the report."""
...
def _risk_badge_row(risk, risk_color, styles, dark_panel=True):
    """A bold, high-contrast pill showing the overall risk rating.

    Used on both the cover and the end page so the headline number of the
    report -- the risk rating -- is impossible to miss.
    """
...
def _cover_story(report, styles):
    ...
    story.append(Paragraph(
        "Automated Vulnerability, Technology &amp;<br/>Security Standards Evaluation",
...
# ============================================================
# Risk overview cards (fixed to avoid label/number overlap)
# ============================================================

def _risk_summary_table(summary, styles):
    # Solid, filled colour cards: strong colour-on-white contrast for the
    # label and a large white numeral, instead of a colour-on-tint pairing
    # that reads faint at a glance.
    items = [
...
def _standards_table(report, styles):
    ...
    if not rows:
        return Paragraph("No security-standard mappings were available.", styles["body"])
...
def _finding_elements(index, finding, styles):
    ...
    header = Table(
        [[
            ...
            Paragraph(f"<font color='{WHITE.hexval()}'><b>{_html(severity).upper()}</b></font>", styles["center"]),
        ]],
    ...
    details = [
        ...
        ("Standards", " | ".join(standards) if standards else None),
    ...
    # Keep each finding's header + body together where it reasonably fits,
    # so a card is not split with only its title stranded at a page bottom.
...
# ============================================================
# Numbered document template (drives the two-pass TOC build)
# ============================================================

class _ReportDocTemplate(BaseDocTemplate):
    """Records heading positions so TableOfContents can resolve page numbers."""
...
def export_pdf(
    ...
    """
    Export a professional WebSET security assessment PDF.
    ...
    intro = (
        ...
        f"The assessment combined a dynamic start scan of the live application with a "
        f"platform/technology evaluation, mapping identified issues to industry-standard "
        f"classification frameworks (CWE, WASC, OWASP, NIST and SANS) where applicable. "
        f"A total of <b>{total}</b> finding(s) were identified, resulting in an overall "
    ...
    for line in executive.splitlines():
        ...
        if not line:
            continue
        if line.startswith("\u2022"):
            story.append(Paragraph(f"&bull; {_html(line[1:].strip())}", styles["body"]))
        else:
    ...
    story.append(Paragraph(
        ...
        "the underlying technology stack for outdated or misconfigured components. Findings were "
        "cross-referenced against CWE, WASC, OWASP, NIST SP 800-53 and SANS reference material "
    ...
    story.append(Spacer(1, 6 * mm))
    story.append(Paragraph("2.3 Security Standards Mapping", styles["subsection"]))
    story.append(_standards_table(report, styles))

    story.append(PageBreak())

    # ========================================================
    # 3. Findings - Start Scan
    # ========================================================
    ...
    if scan_findings:
        for index, finding in enumerate(scan_findings, start=1):
            story.extend(_finding_elements(index, finding, styles))
    else:
    ...
    story.append(Paragraph("4. Platform Evaluation \u2014 Get Stack", styles["section"]))
    ...
    # --------------------------------------------------------
    # Build document (two passes so the Table of Contents can
    # resolve page numbers, final pass rendered with page totals)
    # --------------------------------------------------------
