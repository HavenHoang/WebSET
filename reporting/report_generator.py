...
def generate_report(
    ...
    stack_findings = Get Stack platform evaluation (Tech Stack)
    """
    ...
    for f in stack_findings:
        if f.get("cwe_id"):
            cwe_summary[f["cwe_id"]] = cwe_summary.get(f["cwe_id"], 0) + 1
        if f.get("owasp"):
            owasp_summary[f["owasp"]] = owasp_summary.get(f["owasp"], 0) + 1
        if f.get("nist"):
            nist_summary[f["nist"]] = nist_summary.get(f["nist"], 0) + 1
        if f.get("sans"):
            sans_summary[f["sans"]] = sans_summary.get(f["sans"], 0) + 1
    ...
    lines.append("• Mapped to CWE, WASC, OWASP, NIST and SANS where available")
